...
class SolvingTheProblem {
    ...
    fun `running part one`() {
        val result = processOpCode(puzzleInputPartOne).split(",").first()
    ...
    fun `running part two`() {
        ...
        for (noun in 0..99) {
            for (verb in 0..99) {
                ...
                val output = processOpCode(program).split(",").first()
                if (output == "19690720") {
    ...
    fun `1,0,0,0,99 becomes 2,0,0,0,99 (1 + 1 = 2)`() {
        ...
        Assertions.assertThat(processOpCode(input)).isEqualTo(output)
    }

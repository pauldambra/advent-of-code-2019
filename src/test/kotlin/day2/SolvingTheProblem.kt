...
class SolvingTheProblem {
    ...
    fun `running part one`() {
        val result = ShipsComputer.runProgram(puzzleInputPartOne).split(",").first()
    ...
    fun `running part two`() {
        ...
        for (noun in 0..99) {
            for (verb in 0..99) {
                ...
                val output = ShipsComputer.runProgram(program).split(",").first()
                if (output == "19690720") {
    ...
    fun `1,0,0,0,99 becomes 2,0,0,0,99 (1 + 1 = 2)`() {
        ...
        Assertions.assertThat(ShipsComputer.runProgram(input)).isEqualTo(output)
    }
